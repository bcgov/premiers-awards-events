...
<template>
  <div>
    ...
    <div class="registrations-visual" v-else>
      <DataTable class="p-datatable-sm" exportFilename="Event List" :value="registrations" responsiveLayout="stack"
        breakpoint="780px" :key="dataTableRender" v-model:expandedRows="expandedRows" :rows="10" :paginator="true"
        ref="dt" stripedRows sortField="organization" :sortOrder="1" v-model:filters="filters" filterDisplay="menu"
        :globalFilterFields="['organization', 'guid']" :loading="loading" showGridlines
        ...
        <PrimeColumn header="Guests" :expander="true" headerStyle="width: 3rem" />
      ...
      data.registrar
        ...
        <PrimeColumn field="guestList" header="Guest List" key="guestList" filterField="guestList" sortable
          datatype="numeric">
          <template #body="{ data }">
            {{ data.guests.length }}
          </template>
        ...
        <PrimeColumn field="submitted" header="Submitted?" key="submitted" dataType="boolean">
      ...
      'false-icon pi-times-circle': !data.submitted,
        ...
        <PrimeColumn field="seated" header="All Guests Seated?" key="seated" filterField="allAssigned"
          dataType="boolean">
          <template #body="{ data }"><span>
              <i class="pi pi-check-circle" :class="{
      ...
      'false-icon pi-times-circle':
        ...
        <PrimeColumn header="Options">
          <template #body="{ data }">
            <PrimeButton v-if="data.submitted" icon="pi pi-pencil" label="Bulk Seat Registration"
              class="p-button-rounded p-button-success mr-2 edit-button" @click="editRegistrationTable(data)" />
          </template>
        </PrimeColumn>
        <template #expansion="slotProps">
          <div class="orders-subtable">
            <h5>Guests for {{ slotProps.data.registrar }}</h5>
            <DataTable :value="Object.values(slotProps.data.details)" responsiveLayout="scroll">
      ...
      lookup("attendancetypes", data.attendancetype)
              ...
              <PrimeColumn field="accessibility" header="Accessibility Accomodations Required">
                <template #body="{ data }">
              ...
              <PrimeColumn field="tabledetails.tablename" header="Seated?">
                <template #body="{ data }">
                  <router-link v-if="data.table" :to="`/admin/table/${data.table}`">{{ data.tabledetails.tablename
                    }}</router-link>
              ...
              <PrimeColumn field="table" header="Edit Table">
                <template #body="{ data }">
                  <span v-if="slotProps.data.submitted === false">Must Submit Registration</span>
                  <span v-else>
                    <PrimeButton icon="pi pi-pencil" :label="data.tabledetails ? 'Edit Table' : 'Add Table'"
                      class="p-button-rounded p-button-success mr-2 edit-button" @click="editGuestTable(data)" />
                  </span>
                </template>
              </PrimeColumn>
              <PrimeDialog v-model:visible="tableDetailsDialog" :style="{ width: '50rem', margin: '5rem' }"
                header="Table Details" :modal="true" class="p-fluid registration-dialog">
                <div class="dropdown">
                  <label for="table">Table</label>
                  <DropDown v-bind:class="{ 'p-invalid': v$.table.$error }" id="table" v-model="guest.table" :options="tables.filter(
      (each) => each.tablecapacity > each.guests.length
    )
      " name="table" optionLabel="tablename" optionValue="_id" placeholder="Select the table." />
                  <small v-if="v$.table.$error" class="p-error" id="table-help">Please select the table.</small>
                </div>
                <template #footer>
                  <PrimeButton label="Cancel" icon="pi pi-times" class="p-button-text"
                    @click="tableDetailsDialog = false" />
                  <PrimeButton label="Confirm Table" icon="pi pi-check" class="p-button-text" @click="editTable" />
                </template>
              </PrimeDialog>
            </DataTable>
          </div>
        </template>
        <PrimeDialog v-model:visible="multiTableDialog" :style="{ width: '50rem', margin: '5rem' }"
          header="Table Details" :modal="true" class="p-fluid registration-dialog">
          <div class="dropdown">
            <label for="table">Table</label>
            <DropDown v-bind:class="{ 'p-invalid': v$.table.$error }" id="table" v-model="registration.table" :options="tables.filter((each) => each.tablecapacity > each.guests.length)
      " name="table" optionLabel="tablename" optionValue="_id" placeholder="Select the table." />

            <small v-if="v$.table.$error" class="p-error" id="table-help">Please select the table{{
      registration.guests.length >
        10 ?
        's' : null }}.</small>
            <CheckBox v-model="overRideSeating" :binary="true" name="seatingoverride" inputID="seatingoverride" />
            <label for="seatingoverride" class="ml-2"> Remove guests from current seats and reassign to available seats
              at the
              selected table. This cannot be undone. </label>
            <div v-if="overRideSeating">
              <label for="seatingStartCount">Start at guest:</label>
              <InputText id="seatingStartCount" aria-describedby="seatingStartCount-help"
                v-model="guestSeatStartCount" />
              <small id="seatingStartCount-helphelp">Enter the number for the guest on the registration you'd like to
                begin
                seating.</small>
            </div>
          </div>
          <template #footer>
            <PrimeButton label="Cancel" icon="pi pi-times" class="p-button-text" @click="multiTableDialog = false" />
            <PrimeButton :disabled="!registration.table" label="Confirm Table" icon="pi pi-check" class="p-button-text"
  ...
  setup(props, { emit }) {
    ...
    const multiSeat = async (data) => {
      ...
      async function seatGuests(selectedTableID) {
        const tablesList = tables.value.filter((each) => each["_id"] === selectedTableID)[0];
        const tableCapacity =
        ...
        try {
          ...
          let guestCount = 0;
          const subGuestList = guests.slice(guestSeatStartCount.value === 0 ? 0 : guestSeatStartCount.value - 1);
