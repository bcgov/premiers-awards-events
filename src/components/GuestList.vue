  ...
  <div>
    ...
    <div v-else>
      ...
      'dietary',
        ...
        <PrimeColumn
          v-if="currentUrl.includes('/admin/table/') || currentUrl.includes('/admin/edit/') || (currentUrl.includes('/registration/') && !isSubmitted() && (settingsStore.getIsSalesOpen || (userStore.getUser.roles.includes('super-administrator') || userStore.getUser.roles.includes('administrator'))))"
        ...
        <PrimeColumn v-if="currentUrl.includes('/admin/edit/') || currentUrl.includes('/registration/')"
      ...
      10) : `N/A` }}
        ...
        <PrimeColumn field="organization" header="Organization" key="organization">
          <template #body="{ data }">
            {{
      lookup("organizations", data.organization) || data.organization
    }} </template><template #filter="{ filterModel }" v-if="adminView">
            <DropDown v-model="filterModel.value" :options="organizationsFilter" optionLabel="text" placeholder="Any"
              class="p-column-filter" :showClear="true" @change="(event) => {
      ...
      ">
            ...
            <InputText v-if="!dropdownSelected" type="text" v-model="filterModel.value" class="p-column-filter"
              placeholder="Search by Text" />
          </template>
          <template #filterfooter>
            <PrimeButton type="button" @click="() => {
      ...
      ">Reset Selector Options</PrimeButton>
        ...
        <PrimeColumn field="registrar" header="Registrar" key="registrar">
          <template #body="{ data }">
            {{ data.registration.primarycontact }} </template><template #filter="{ filterModel }" v-if="adminView">
            <InputText type="text" v-model="filterModel.value" class="p-column-filter"
        ...
        <PrimeColumn field="firstname" header="First Name" key="firstname">
          <template #body="{ data }"> {{ data.firstname }} </template><template #filter="{ filterModel }"
            v-if="adminView">
            <InputText type="text" v-model="filterModel.value" class="p-column-filter"
              placeholder="Search by First Name" />
          </template>
        ...
        <PrimeColumn field="lastname" header="Last Name" key="lastname">
          <template #body="{ data }"> {{ data.lastname }} </template><template #filter="{ filterModel }"
            v-if="adminView">
            <InputText type="text" v-model="filterModel.value" class="p-column-filter"
              placeholder="Search by Last Name" />
          </template>
        </PrimeColumn>
        <PrimeColumn field="attendancetype" header="Attendance Type" key="attendancetype">
          <template #body="{ data }">
            {{ lookup("attendancetypes", data.attendancetype) }} </template><template #filter="{ filterModel }"
            v-if="adminView">
            <DropDown v-model="filterModel.value" :options="attendancetypesFilter" optionLabel="text" placeholder="Any"
      ...
      data.custompronouns
        ...
        : data.pronouns.toString()
            ...
            <DropDown v-model="filterModel.value" :options="pronounsFilter" optionLabel="text" placeholder="Any"
              class="p-column-filter" :showClear="true">
              <template #value="slotProps">
                <div v-if="slotProps.value">
                  <div>
        ...
        <PrimeColumn field="accessibility" header="Accessibility Requirements" key="accessibility">
          <template #body="{ data }">
            {{
      lookupLoop("accessibilityoptions", data.accessibility)
    }} </template><template #filter="{ filterModel }" v-if="adminView">
            <DropDown v-model="filterModel.value" :options="accessibilityFilter" optionLabel="text" placeholder="Any"
              class="p-column-filter" :showClear="true">
              <template #value="slotProps">
                <div v-if="slotProps.value">
                  <div>
        ...
        <PrimeColumn field="supportingfinalist" header="Supporting Finalist" key="supportingfinalist">
          <template #body="{ data }"> {{ data.supportingfinalist }} </template><template #filter="{ filterModel }"
            v-if="adminView">
            <InputText type="text" v-model="filterModel.value" class="p-column-filter"
              placeholder="Search by Supporting Finalist" />
          </template>
        </PrimeColumn>
        <PrimeColumn field="dietary" header="Dietary Requirements" key="dietary">
          <template #body="{ data }">
            {{ lookupLoop("dietaryoptions", data.dietary) }} </template><template #filter="{ filterModel }"
            v-if="adminView">
            <DropDown v-model="filterModel.value" :options="dietaryFilter" optionLabel="text" placeholder="Any"
              class="p-column-filter" :showClear="true">
              <template #value="slotProps">
                <div v-if="slotProps.value">
                  <div>
        ...
        <PrimeColumn field="notes" header="Notes:" key="notes" filterField="hasNotes">
        ...
        <PrimeColumn v-if="adminView" field="tabledetails.tablename" header="Table" key="tabledetails.tablename"
          dataType="boolean" filterField="assignedTable" sortable>
      ...
      formatTime(data.createdAt)
        ...
        <PrimeColumn v-if="adminView" field="updatedAt" header="Updated:" key="updatedAt" :sortable="true"
          ...
          <template #body="{ data }">
            ...
            {{ formatTime(data.updatedAt) }} </template><template #filter="{ filterModel }">
            <PrimeCalendar v-model="filterModel.value" dateFormat="mm/dd/yy" placeholder="mm/dd/yyyy" />
          </template>
        </PrimeColumn>
    ...
    <div>
      <PrimeDialog v-model:visible="guestDialog" :style="{ width: '50rem', margin: '5rem' }" header="Guest Details"
        :modal="true" class="p-fluid" @show="() =>
    (guest.organization =
      lookup('organizations', guest.organization) || guest.organization)
      ">
        <div class="dropdown">
          <label for="organization">Organization:</label>
          <AutoComplete v-bind:class="{ 'p-invalid': v$.organization.$error }" v-model="guest.organization"
            :dropdown="true" :suggestions="filteredOrganizations" @complete="searchOrganization($event)"
            placeholder="Select an organization or enter manually" optionLabel="text" field="text" modelValue="text"
            :disabled="guest.hasexternalorganization">
            <template #option="slotProps">
              <div>{{ slotProps.option.text }}</div>
            </template>
          </AutoComplete>
          <small v-if="v$.organization.$error" class="p-error" id="organization-help">Please select your
            organization.</small>
        </div>

        <div class="field-checkbox">
          <CheckBox name="hasexternalorganization" :binary="true" v-model="guest.hasexternalorganization"
            @change="guest.organization = ''" />
          <label for="hasexternalorganization">Guest is from an external organization
          </label>
        </div>

        <div class="field-text" v-if="guest.hasexternalorganization">
          <label for="externalorganization">Please enter external organization's name:</label>
          <InputText id="externalorganization" type="externalorganization" aria-describedby="xternalorganization-help"
            v-model.trim="guest.organization" placeholder="External organization name" />
        </div>

        <div class="field-text">
          <label for="firstname">First Name</label>
          <InputText v-bind:class="{ 'p-invalid': v$.firstname.$error }" id="firstname" v-model.trim="guest.firstname"
            required="true" name="firstname" title="First Name" autofocus />
          <small v-if="v$.firstname.$error" class="p-error" id="firstname-help">Please enter guest's first name.</small>
        </div>
        <div class="field-text">
          <label for="lastname">Last Name</label>
          <InputText v-bind:class="{ 'p-invalid': v$.lastname.$error }" id="lastname" v-model.trim="guest.lastname"
            required="true" name="lastname" title="Last Name" autofocus />
          <small v-if="v$.lastname.$error" class="p-error" id="lastname-help">Please enter guest's last name.</small>
        </div>

        <div class="dropdown">
          <label for="attendancetype">Attendance Type:</label>
          <DropDown v-bind:class="{ 'p-invalid': v$.attendancetype.$error }" id="attendancetype"
            v-model="guest.attendancetype" :options="attendancetypes" optionLabel="text" optionValue="value"
            name="attendancetype" title="Attendance Type" placeholder="Select the type of attendance for this guest" />
          <small v-if="v$.attendancetype.$error" class="p-error" id="attendancetype-help">Please select the attendance
            type
            for this guest.</small>
        </div>
        <label for="pronouns">Pronouns:</label>
        <div class="checkbox-group">
          <div v-for="each of pronouns" :key="each.key" name="pronouns" class="field-checkbox">
            <CheckBox :id="each.key" name="pronouns" :value="each.value" v-model="guest.pronouns" />
            <label :for="each.key">{{ each.text }}</label>
          </div>
          <div class="field-checkbox">
            <CheckBox name="hascustompronouns" :binary="true" v-model="guest.hascustompronouns"
        ...
        <div class="field-text" v-if="guest.hascustompronouns">
          ...
          <InputText id="custompronouns" type="custompronouns" aria-describedby="custompronouns-help"
            v-model="guest.custompronouns" placeholder="Custom pronouns" />
        </div>
        <label for="accessibility">Accessibility Requirements:</label>
        <div class="checkbox-group">
          <div v-for="each of accessibility" :key="each.key" name="accessibility" class="field-checkbox">
            <CheckBox :id="each.key" name="accessibility" :value="each.value" v-model="guest.accessibility" />
            <label :for="each.key">{{ each.text }}</label>
          </div>
        </div>
        <label for="dietary">Dietary Requirements:</label>
        <div class="checkbox-group">
          <div v-for="each of dietary" :key="each.key" class="field-checkbox" name="dietary">
            <CheckBox :id="each.key" name="dietary" :value="each.value" v-model="guest.dietary" />
            <label :for="each.key">{{ each.text }}</label>
          </div>
        </div>
        ...
        <div class="dropdown">
          ...
          <DropDown id="supportingfinalist" v-model="guest.supportingfinalist" :options="supportingfinalist"
            optionLabel="text" optionValue="value" name="supportingfinalist" title="Supporting Finalist"
        ...
        <div class="field-text">
          ...
          <InputText id="guestnotes" v-model.trim="guest.notes" name="guestnotes" title="Notes" autofocus />
        </div>

        <template #footer>
          <PrimeButton label="Cancel" icon="pi pi-times" class="p-button-text" @click="hideDialog" />
          <PrimeButton label="Save" icon="pi pi-check" class="p-button-text" @click="saveGuest" />
        </template>
      </PrimeDialog>
      <PrimeDialog v-model:visible="deleteGuestDialog" :style="{ width: '450px' }" header="Confirm" :modal="true">
        <div class="confirmation-content">
          <i class="pi pi-exclamation-triangle mr-3" style="font-size: 2rem" />
          <span v-if="guest">Are you sure you want to
            {{ !tableID && !guest.table ? "delete" : "remove" }}
            <b>{{ guest.firstname }} {{ guest.lastname }}</b>?</span>
        </div>
        <template #footer>
          <PrimeButton label="No" icon="pi pi-times" class="p-button-text" @click="deleteGuestDialog = false" />
          <PrimeButton v-if="!tableID && !guest.table" label="Yes" icon="pi pi-check" class="p-button-text"
            @click="deleteGuest" />

          <PrimeButton v-else icon="pi pi-trash" label="Remove from Table"
            class="p-button-rounded p-button-warning delete-button" @click="removeGuest(guest.table)" />
        </template>
      </PrimeDialog>
    </div>
  </div>
...
export default {
  ...
  setup(props) {
        ...
        await new Promise((resolve) => setTimeout(resolve, 1))
          ...
          .then(() => {
            guests.value.forEach((guest, index) => {
              guest.seat = currentUrl.includes('/admin/guests') ? guest.seat : ref(index + 1);
              currentUrl.includes('/admin/table/') ? guestStore.updateGuest(guest._id, guest) : null;
    ...
    const exportCSV = () => {
      const requestedSeat = currentUrl.includes("/admin/edit/") || currentUrl.includes("/registration/");
      dt.value.value.map((each) => {
        const seat = {
          seat: requestedSeat ? (each.index ? `${Math.ceil(each.index / 10)} - ${each.index % 10 || 10}` : null) : (each.seat ? (`${each.seat % 10 || 10}`) : null)
        };
        const index = {
          index: requestedSeat ? (each.index ? `Table ${Math.ceil(each.index / 10)} - ${each.index % 10 || 10}` : null) : (each.seat ? (`${each.seat % 10 || 10}`) : null)
        };
        const organization = {
          organization:
    ...
    const onRowReorder = (event) => {
      ...
      if (tableSeating) {
        ...
        tableStore.updateTable(tableID, newTableData).then(newTableData.guests.forEach(data => {
        ...
        const newRegistrationData = { ...registration.value, guests: event.value, }
        financialStore.registerFinancialInformation(newRegistrationData, registrationID)
      }
